...
def convert_salidas_to_xls_task(salidas):
    ...
    for row in rows:
        ...
        for col_num in range(len(row)):
            ...
            if col_num == 12:
                (ws.cell(row = row_num, column = col_num+1, value=row[col_num])).style = money_style
        ws.cell(row=row_num, column=len(row) + 1, value=f'=M{row_num} * L{row_num}').style = money_style
    
    (ws.cell(column = columna_max , row = 3, value=f'=SUM(N2:N{row_num})')).style = money_resumen_style
