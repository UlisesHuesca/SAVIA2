...
def update_salida(request):
    ...
    print('orden:',producto.articulos.orden)
    ...
    if action == "remove":
        ...
        if item.entrada and item.entrada != 0:
            ...
            entrada.cantidad_por_surtir += item.cantidad 
            inv_del_producto.cantidad_entradas += item.cantidad
...
def render_pdf_view( pk):
   ...
   #Azul Vordcab
    ...
    if orden.autorizar == False:
        ...
        c.setFillColor(rojo)
        c.setFont('Helvetica-Bold',14)
        c.drawString(390,650, 'NO AUTORIZADA AÚN')
    c.setFillColor(black)
    c.setFont('Helvetica',9)
...
def generate_excel_report2(salidas):
    ...
    for salida in salidas:
        ...
        rows = [
            salida.vale_salida.folio,
            salida.vale_salida.solicitud.folio,
            salida.vale_salida.created_at, #.strftime('%Y-%m-%d'),  # Formatea la fecha para la celda
            solicitante,
