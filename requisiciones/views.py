...
def update_devolucion_apartado(request):
    ...
    if action == "add":
        ...
        if cantidad_total < 0:
            ...
            #Se le resta a la cantidad de artículos para surtir o a la salida cuando es salida, de hecho la salida debe marcarse como cancelada en algún momento
            #Si yo devuelvo la salida tendría que aumentarse el inventario en cantidad apartada
...
def update_devolucion_inventario(request):
    data= json.loads(request.body)
    action = data["action"]
    cantidad = decimal.Decimal(data["val_cantidad"])
    devolucion = data["devolucion"]
    ...
    comentario = data["comentario"]
    devolucion = Devolucion.objects.get(id = devolucion)
    producto = Salidas.objects.get(id = producto_id)
...
def autorizar_devolucion(request, pk):
    ...
    try:
        entrada = EntradaArticulo.objects.get(id=salida.entrada)
    except EntradaArticulo.DoesNotExist:
        # Manejar el caso en que no hay una entrada asociada (opcional)
        entrada = 0
    print(entrada)
    #unica_salida = salida.first()
    #vale_salida = ValeSalidas.objects.get(id = unica_salida.vale_salida.id)
    
    if request.method == 'POST' and 'btnAutorizar' in request.POST:
       
        for producto in productos:
            if devolucion.tipo.nombre == "INVENTARIO":
            ...
            elif devolucion.tipo.nombre == "PROVEEDOR": #NO CONFORMIDAD, DEVOLUCION A PROVEEDOR
                producto_surtir = ArticulosparaSurtir.objects.get(articulos = producto.producto.articulos)
                #producto_surtir = Salidas.objects.get(id=devolucion.salida.id)
                #regresa_inventario = salida.cantidad - devolucion.cantidad
                inv_del_producto = Inventario.objects.get(producto = producto_surtir.articulos.producto.producto)
                inv_del_producto._change_reason = f'Esta es una devolucion de una NC {devolucion.id}, es decir se va a devolver a proveedor'
                entrada.cantidad_por_surtir = entrada.cantidad_por_surtir + producto.cantidad #Se devuelve a la entrada la cantidad que había surtido
                producto_surtir.cantidad_requisitar = producto_surtir.cantidad_requisitar + producto.cantidad
                producto_surtir.requisitar = True
                producto_surtir.save()
                entrada.cantidad = entrada.cantidad - producto.cantidad 
                entrada.cantidad_por_surtir = entrada.cantidad_por_surtir - entrada.cantidad_por_surtir 
                if entrada.cantidad == 0:
                    entrada.cancelada = True
                entrada.save()
                artcompra = ArticuloComprado.objects.get(id=entrada.articulo_comprado.id)
                compra = Compra.objects.get(id=artcompra.oc.id)

                artcompra.cantidad_pendiente = artcompra.cantidad_pendiente + producto.cantidad
                compra.entrada_completa = False
                artcompra.entrada_completa = False
                artcompra.save()
                compra.save()        
            else:
                producto_surtir = ArticulosparaSurtir.objects.get(articulos = producto.producto.articulos)
...
def devolucion_material_salida(request, pk):
    ...
    print('productos:',productos_sel)
    devolucion, created = Devolucion.objects.get_or_create(almacenista = usuario,complete = False, solicitud=orden, salida =salida)
    productos = Devolucion_Articulos.objects.filter(vale_devolucion = devolucion)
    cantidad_items = productos.count()
    form = DevolucionArticulosForm()
    form2 = DevolucionForm()

    form2.fields['tipo'].queryset = Tipo_Devolucion.objects.exclude(id=1)
    ...
    if request.method == 'POST':
        ...
        <html>
            ...
            <body>
                ...
                <p>por {requi.requi_autorizada_por.staff.staff.first_name} {requi.requi_autorizada_por.staff.staff.last_name}.</p>
...
def render_pdf_view(request, pk):
   ...
   #Azul Vordcab
    ...
    c.setFillColor(black)
    c.drawCentredString(230,high-190, orden.staff.staff.staff.first_name +' '+ orden.staff.staff.staff.last_name)
...
def render_salida_pdf(request, pk):
    ...
    c.drawCentredString(400,proyecto_y - 50,'Recibió')
    c.drawCentredString(400,proyecto_y - 60, vale.material_recibido_por.staff.staff.first_name +' '+vale.material_recibido_por.staff.staff.last_name)
